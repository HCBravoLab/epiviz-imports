...
epiviz.plugins.charts.CustomScatterPlot.prototype._drawCircles = function(data, dimx, dimy,key) {
    ...
    if (minX == CustomSetting.DEFAULT) { minX = d3.min(data, function(d){
    	return d[dimx];
    }); }
    if (minY == CustomSetting.DEFAULT) { minY = d3.min(data, function(d){
    	return d[dimy];
    }); }
    if (maxX == CustomSetting.DEFAULT) { maxX = d3.max(data, function(d){
    	return d[dimx];
    }); }
    if (maxY == CustomSetting.DEFAULT) { maxY = d3.max(data, function(d){
    	return d[dimy];
    }); }
    ...
    var items = [];
    var maxGroupItems = 1;
    var seriesIndex = 1; // Assume only 1 pair of datax and datay
    for (var i = 0; i < data.length; ++i) {
        ...
        uiObj = new epiviz.ui.charts.ChartObject(
            sprintf('scatter_%s_%s', seriesIndex, data[i][key]),
            //data[i][key],
            null,
            null,
            //cellX.rowItem.start(),
